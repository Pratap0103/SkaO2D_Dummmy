...
const GetLoading2nd = () => {
  ...
  const { getFilteredData, updateLoading2nd } = useDataStore();
  ...
  const pendingData = loadingData.filter(item => item.loading1stCompleted && !item.loading2ndCompleted);
  const historyData = loadingData.filter(item => item.loading2ndCompleted);
  ...
  const handleSubmitLoading2nd = (id) => {
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredPendingData.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button
                          onClick={() => handleSubmitLoading2nd(item.id)}
          ...
          {activeTab === 'history' && (
            <div className="overflow-x-auto">
              ...
              {filteredHistoryData.length === 0 && (
                <div className="px-6 py-12 text-center">
                  <p className="text-gray-500">No loading 2nd history found.</p>
                </div>
              )}
            </div>
          )}
