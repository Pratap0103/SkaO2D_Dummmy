...
const GetLoading1st = () => {
  ...
  const { getFilteredData, updateLoading1st } = useDataStore();
  ...
  // Initialize loading data from completed tyre weight data
  useEffect(() => {
    const completedTyreWeightData = tyreWeightData.filter(item => item.tyreWeightStatus === 'completed');
    completedTyreWeightData.forEach(tyreWeightItem => {
      const existsInLoading = loadingData.find(loadingItem => loadingItem.id === tyreWeightItem.id);
      if (!existsInLoading) {
        // Add to loading data
        useDataStore.getState().loadingData.push({
          ...tyreWeightItem,
          loading1stStatus: '',
          loading1stCompleted: false,
          loading2ndStatus: '',
          loading2ndCompleted: false
        });
      }
    });
  }, [tyreWeightData]);

  const pendingData = loadingData.filter(item => !item.loading1stCompleted);
  const historyData = loadingData.filter(item => item.loading1stCompleted);
  ...
  const handleSubmitLoading1st = (id) => {
    const status = loadingStatus[id];
    if (!status) {
      toast.error('Please select loading status');
      return;
    }
    
    updateLoading1st(id, status);
    toast.success('Loading 1st status updated successfully!');
  ...
  const uniqueParties = [...new Set(loadingData.map(item => item.partyName))];
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredPendingData.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button
                          onClick={() => handleSubmitLoading1st(item.id)}
                          disabled={!selectedItems[item.id] || !loadingStatus[item.id]}
          ...
          {activeTab === 'history' && (
            <div className="overflow-x-auto">
              ...
              {filteredHistoryData.length === 0 && (
                <div className="px-6 py-12 text-center">
                  <p className="text-gray-500">No loading 1st history found.</p>
                </div>
              )}
            </div>
          )}
